...
def select_model(args, device):
    ...
    if model_id == 0:
        ...
        name, data_range = f"{model_id:02}_SCMSR", 1.0 # You can choose either 1.0 or 255.0 based on your own model
...
def run(model, model_name, data_range, tile, logger, device, args, mode="test"):
    ...
    # dataset path
    # --------------------------------
    data_path = sorted(glob.glob(os.path.join(args.data_dir, "DIV2K_LSDIR_test_LR/*.png")))
    ...
    for img_lr in data_path:

        # --------------------------------
        # (1) img_lr
        # --------------------------------
        img_name, ext = os.path.splitext(os.path.basename(img_lr))
        img_name = img_name.replace("x4", "")  # 移除文件名中的 x4
    ...
    logger.info("------> Average runtime of (test) is : {:.6f} milliseconds".format(results[f"{mode}_ave_runtime"]))
...
def main(args):
    ...
    if True:
        ...
        # print(f"Current model_name: {model_name}")
        # print(results.keys())  # 看看 results 里有哪些模型名称
        # print(results.get(model_name, {}).keys())  # 看看 model_name 对应的内容
    ...
    s = fmt.format("Model", "Test Time [ms]", "Params [M]", "FLOPs [G]", "Acts [M]", "Mem [M]", "Conv")
    ...
    for k, v in results.items():
        ...
        s += fmt.format(k, test_time, num_param, flops, acts, mem, conv)
